...
interface CSRFrameworkProps {
  title: string;
  description: string;
  details: CSRDetail[];
}

const CSRFramework: React.FC<CSRFrameworkProps> = ({ 
  title, 
  description, 
  details = []
}) => {
  ...
  const getIconByTitle = (title: string) => {
    ...
    if (titleLower.includes('scholarship') || titleLower.includes('educat')) {
      return <GraduationCap className="w-8 h-8 text-company-orange" />;
    } else if (titleLower.includes('health')) {
      return <Stethoscope className="w-8 h-8 text-company-orange" />;
    } else if (titleLower.includes('school') || titleLower.includes('feed')) {
      return <School className="w-8 h-8 text-company-orange" />;
    } else if (titleLower.includes('olympics') || titleLower.includes('sport')) {
      return <Footprints className="w-8 h-8 text-company-orange" />;
    } else if (titleLower.includes('infrastructure') || titleLower.includes('building')) {
      return <Building2 className="w-8 h-8 text-company-orange" />;
    }
    return <GraduationCap className="w-8 h-8 text-company-orange" />;
  };
          ...
          {details.map((initiative, index) => (
            ...
            >
              <Card className="h-full hover:shadow-lg transition-all duration-300 group">
                <CardContent className="p-6">
                  <div className="bg-company-royal/5 w-16 h-16 rounded-lg flex items-center justify-center mb-6 group-hover:bg-company-royal/10 transition-colors">
                    {getIconByTitle(initiative.title)}
                  </div>
                  <h3 className="text-xl font-semibold mb-3 text-company-royal">
                    {initiative.title}
                  </h3>
