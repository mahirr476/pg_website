// // src/components/business/BusinessHero.tsx
// 'use client';
// import { motion } from 'framer-motion';
// import Image from 'next/image';
// import { Card, CardContent } from '@/components/ui/card';

// interface BusinessHeroProps {
//   data: {
//     title: string;
//     heroImage: string;
//     heroStatement: string;
//     description: string;
//   };
// }

// const BusinessHero = ({ data }: BusinessHeroProps) => {
//   return (
//     <section className="relative min-h-[80vh] flex items-center">
//       <div className="absolute inset-0">

//         <div className="absolute inset-0 bg-gradient-to-r from-black/80 to-transparent" />
//       </div>

//       <div className="container mx-auto px-4 relative z-10">
//         <motion.div
//           initial={{ opacity: 0, x: -20 }}
//           animate={{ opacity: 1, x: 0 }}
//           transition={{ duration: 0.8 }}
//           className="max-w-2xl text-white"
//         >
//           <motion.h1 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6 }}
//             className="text-6xl font-bold mb-6"
//           >
//             {data.title}
//           </motion.h1>
//           <motion.p 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6, delay: 0.2 }}
//             className="text-2xl text-gray-100 mb-8"
//           >
//             {data.heroStatement}
//           </motion.p>
//           <motion.p 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6, delay: 0.4 }}
//             className="text-lg text-gray-200 leading-relaxed"
//           >
//             {data.description}
//           </motion.p>
//         </motion.div>
//       </div>

//       <motion.div
//         initial={{ opacity: 0, y: 20 }}
//         animate={{ opacity: 1, y: 0 }}
//         transition={{ duration: 0.8, delay: 0.4 }}
//         className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-white to-transparent"
//       />
//     </section>
//   );
// };

// export default BusinessHero;


// 'use client';

// import { motion } from 'framer-motion';

// interface BusinessHeroProps {
//   data: {
//     title: string;
//     shortDes: string;
//     longDes: string;
//   };
// }

// const BusinessHero = ({ data }: BusinessHeroProps) => {
//   return (
//     <section className="relative min-h-[80vh] flex items-center">
//       <div className="absolute inset-0">
//         {/* Background image div instead of Next.js Image */}
//         <div 
//           className="absolute inset-0 bg-cover bg-center"
//           style={{ backgroundImage: "url('/images/business/business-hero-bg.jpg')" }}
//         />
//         <div className="absolute inset-0 bg-gradient-to-r from-black/80 to-transparent" />
//       </div>

//       <div className="container mx-auto px-4 relative z-10">
//         <motion.div
//           initial={{ opacity: 0, x: -20 }}
//           animate={{ opacity: 1, x: 0 }}
//           transition={{ duration: 0.8 }}
//           className="max-w-2xl text-white"
//         >
//           <motion.h1 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6 }}
//             className="text-6xl font-bold mb-6"
//           >
//             {data.title}
//           </motion.h1>
//           <motion.p 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6, delay: 0.2 }}
//             className="text-2xl text-gray-100 mb-8"
//           >
//             {data.shortDes}
//           </motion.p>
//           <motion.div 
//             initial={{ opacity: 0, y: 20 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.6, delay: 0.4 }}
//             className="text-lg text-gray-200 leading-relaxed prose prose-invert"
//             dangerouslySetInnerHTML={{ __html: data.longDes }}
//           />
//         </motion.div>
//       </div>

//       <motion.div
//         initial={{ opacity: 0, y: 20 }}
//         animate={{ opacity: 1, y: 0 }}
//         transition={{ duration: 0.8, delay: 0.4 }}
//         className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-white to-transparent"
//       />
//     </section>
//   );
// };

// export default BusinessHero;

'use client';

import { motion } from 'framer-motion';

interface BusinessHeroProps {
  data: {
    title: string;
    shortDes: string;
    longDes: string;
    img: string;
  };
}

const BusinessHero = ({ data }: BusinessHeroProps) => {
  // Process image path for proper handling
  const imagePath = data.img
    ? `/${data.img.replace(/^public\//, '')}`
    : '/images/business/business-hero-bg.jpg';

  return (
    <section className="relative min-h-[85vh] flex items-center">
      <div className="absolute inset-0">
        {/* Simple background image */}
        <div 
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url('${imagePath}')` }}
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black/80 to-black/40" />
      </div>

      <div className="container mx-auto px-4 relative z-10">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 1 }}
          className="max-w-2xl text-white"
        >
          <motion.h1 
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="text-5xl md:text-6xl font-bold mb-8"
          >
            {data.title}
          </motion.h1>
          
          <motion.p 
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            className="text-xl md:text-2xl text-gray-100 mb-8"
          >
            {data.shortDes}
          </motion.p>
          
          <motion.div 
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.4 }}
            className="text-base md:text-lg text-gray-200 leading-relaxed prose prose-invert"
            dangerouslySetInnerHTML={{ __html: data.longDes }}
          />
        </motion.div>
      </div>

      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1, delay: 0.6 }}
        className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-white to-transparent"
      />
    </section>
  );
};

export default BusinessHero;